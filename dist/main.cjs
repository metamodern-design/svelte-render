...
const readFile = (file) => new Promise((fulfil, reject) => fs$1.readFile(file, 'utf-8', (err, contents) => (err ? reject(err) : fulfil(contents))));
...
const svelteRender = async (context, {
  ...
  if (mode === 'production') {
    const indexEntry = path__default.resolve(context, src, index);
    ...
    const cache = path__default.resolve(context, './.svelte-render/index.js');
    ...
    const template = await fs.readFile(
      path__default.resolve(context, src, 'template.html'),
      'utf8',
    );

    await fs.outputFile(
      path__default.resolve(context, dist, 'index.html'),
      renderHtml(component, template),
    );
  } // else generate minimal index.html

  const clientEntry = path__default.resolve(context, src, client);
  ...
  await clientBundle.write({
    format: 'iife',
    file: path__default.resolve(context, dist, client),
  });
};
